...
provided as request body parameters. `sender_address`, `arbitrary_data`, `screen_config` and all its child fields are
optional. `arbitrary_data` field can contain any desired JSON value. Send request ID can be interpolated into the
