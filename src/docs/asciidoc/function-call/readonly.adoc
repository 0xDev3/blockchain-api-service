...
The `function_params` field is an array of objects which must have `type` and `value` according to the table in
...
The `output_params` field is an array of output parameter types, which correspond to the type name according to the table
...
.Request Schema
include::{snippets}/dev3.blockchainapiservice.model.request.ReadonlyFunctionCallRequest.adoc[]
.Response Schema
include::{snippets}/dev3.blockchainapiservice.model.response.ReadonlyFunctionCallResponse.adoc[]
