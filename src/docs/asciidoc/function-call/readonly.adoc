...
in section <<Available Function Argument Types>>. Structure types are constructed by providing an object instead of type
name, for example `{"type": "struct", "elems": ["uint", "string"]}` is a struct with element types of `uint` and
`string`. Structure nesting is possible by providing the appropriate value in the `elem` array. Any array dimensions of
the structure must be expressed in its `type` field, for example `{"type": "struct[]", "elems": ["uint", "string"]}`
defines an array of structures with element types of `uint` and `string`.
