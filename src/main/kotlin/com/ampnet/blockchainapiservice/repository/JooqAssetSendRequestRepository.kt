...
class JooqAssetSendRequestRepository(private val dslContext: DSLContext) : AssetSendRequestRepository {
    ...
    override fun getById(id: UUID): AssetSendRequest? {
        ...
        return dslContext.selectFrom(AssetSendRequestTable.ASSET_SEND_REQUEST)
            .where(AssetSendRequestTable.ASSET_SEND_REQUEST.ID.eq(id))
            .fetchOne { it.toModel() }
    }
    ...
    override fun getAllByProjectId(projectId: UUID): List<AssetSendRequest> {
        ...
        return dslContext.selectFrom(AssetSendRequestTable.ASSET_SEND_REQUEST)
            .where(AssetSendRequestTable.ASSET_SEND_REQUEST.PROJECT_ID.eq(projectId))
            .orderBy(AssetSendRequestTable.ASSET_SEND_REQUEST.CREATED_AT.asc())
            .fetch { it.toModel() }
    }
    ...
    override fun getBySender(sender: WalletAddress): List<AssetSendRequest> {
        ...
        return dslContext.selectFrom(AssetSendRequestTable.ASSET_SEND_REQUEST)
            .where(AssetSendRequestTable.ASSET_SEND_REQUEST.ASSET_SENDER_ADDRESS.eq(sender))
            .orderBy(AssetSendRequestTable.ASSET_SEND_REQUEST.CREATED_AT.asc())
            .fetch { it.toModel() }
    }
    ...
    override fun getByRecipient(recipient: WalletAddress): List<AssetSendRequest> {
        ...
        return dslContext.selectFrom(AssetSendRequestTable.ASSET_SEND_REQUEST)
            .where(AssetSendRequestTable.ASSET_SEND_REQUEST.ASSET_RECIPIENT_ADDRESS.eq(recipient))
            .orderBy(AssetSendRequestTable.ASSET_SEND_REQUEST.CREATED_AT.asc())
            .fetch { it.toModel() }
    }
    ...
    override fun setTxInfo(id: UUID, txHash: TransactionHash, caller: WalletAddress): Boolean {
        ...
        return dslContext.update(AssetSendRequestTable.ASSET_SEND_REQUEST)
            .set(AssetSendRequestTable.ASSET_SEND_REQUEST.TX_HASH, txHash)
            .set(
                AssetSendRequestTable.ASSET_SEND_REQUEST.ASSET_SENDER_ADDRESS,
                coalesce(AssetSendRequestTable.ASSET_SEND_REQUEST.ASSET_SENDER_ADDRESS, caller)
            )
            .where(
                DSL.and(
                    AssetSendRequestTable.ASSET_SEND_REQUEST.ID.eq(id),
                    AssetSendRequestTable.ASSET_SEND_REQUEST.TX_HASH.isNull()
                )
            )
            .execute() > 0
    }
