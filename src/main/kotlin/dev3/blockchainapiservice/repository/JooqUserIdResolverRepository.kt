...
class JooqUserIdResolverRepository(private val dslContext: DSLContext) : UserIdResolverRepository {
    ...
    override fun getByProjectId(projectId: UUID): UUID? = getUserId(IdType.PROJECT_ID, projectId)
    ...
    override fun getUserId(idType: IdType, id: UUID): UUID? {
        ...
        val projectId = when (idType) {
            IdType.PROJECT_ID ->
                ProjectTable.run { ID.select(id, ID) }

            IdType.ASSET_BALANCE_REQUEST_ID ->
                AssetBalanceRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.ASSET_MULTI_SEND_REQUEST_ID ->
                AssetMultiSendRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.ASSET_SEND_REQUEST_ID ->
                AssetSendRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.AUTHORIZATION_REQUEST_ID ->
                AuthorizationRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.CONTRACT_DEPLOYMENT_REQUEST_ID ->
                ContractDeploymentRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.FUNCTION_CALL_REQUEST_ID ->
                ContractFunctionCallRequestTable.run { ID.select(id, PROJECT_ID) }

            IdType.ERC20_LOCK_REQUEST_ID ->
                Erc20LockRequestTable.run { ID.select(id, PROJECT_ID) }
        }
    ...
    private fun <R : Record> TableField<R, UUID>.select(
        id: UUID,
        projectIdField: TableField<*, UUID>
    ): UUID? =
