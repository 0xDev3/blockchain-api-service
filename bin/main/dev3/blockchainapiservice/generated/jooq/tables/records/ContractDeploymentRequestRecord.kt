/*
 * This file is generated by jOOQ.
 */
package dev3.blockchainapiservice.generated.jooq.tables.records


import com.fasterxml.jackson.databind.JsonNode

import dev3.blockchainapiservice.generated.jooq.tables.ContractDeploymentRequestTable
import dev3.blockchainapiservice.util.Balance
import dev3.blockchainapiservice.util.ChainId
import dev3.blockchainapiservice.util.ContractAddress
import dev3.blockchainapiservice.util.ContractBinaryData
import dev3.blockchainapiservice.util.TransactionHash
import dev3.blockchainapiservice.util.UtcDateTime
import dev3.blockchainapiservice.util.WalletAddress

import java.util.UUID

import org.jooq.Record1
import org.jooq.impl.UpdatableRecordImpl


@Suppress("UNCHECKED_CAST")
open class ContractDeploymentRequestRecord() : UpdatableRecordImpl<ContractDeploymentRequestRecord>(ContractDeploymentRequestTable) {

    var id: UUID
        private set(value): Unit = set(0, value)
        get(): UUID = get(0) as UUID

    var alias: String
        private set(value): Unit = set(1, value)
        get(): String = get(1) as String

    var contractMetadataId: UUID
        private set(value): Unit = set(2, value)
        get(): UUID = get(2) as UUID

    var contractData: ContractBinaryData
        private set(value): Unit = set(3, value)
        get(): ContractBinaryData = get(3) as ContractBinaryData

    var constructorParams: JsonNode
        private set(value): Unit = set(4, value)
        get(): JsonNode = get(4) as JsonNode

    var initialEthAmount: Balance
        private set(value): Unit = set(5, value)
        get(): Balance = get(5) as Balance

    var chainId: ChainId
        private set(value): Unit = set(6, value)
        get(): ChainId = get(6) as ChainId

    var redirectUrl: String
        private set(value): Unit = set(7, value)
        get(): String = get(7) as String

    var projectId: UUID
        private set(value): Unit = set(8, value)
        get(): UUID = get(8) as UUID

    var createdAt: UtcDateTime
        private set(value): Unit = set(9, value)
        get(): UtcDateTime = get(9) as UtcDateTime

    var arbitraryData: JsonNode?
        private set(value): Unit = set(10, value)
        get(): JsonNode? = get(10) as JsonNode?

    var screenBeforeActionMessage: String?
        private set(value): Unit = set(11, value)
        get(): String? = get(11) as String?

    var screenAfterActionMessage: String?
        private set(value): Unit = set(12, value)
        get(): String? = get(12) as String?

    var contractAddress: ContractAddress?
        private set(value): Unit = set(13, value)
        get(): ContractAddress? = get(13) as ContractAddress?

    var deployerAddress: WalletAddress?
        private set(value): Unit = set(14, value)
        get(): WalletAddress? = get(14) as WalletAddress?

    var txHash: TransactionHash?
        private set(value): Unit = set(15, value)
        get(): TransactionHash? = get(15) as TransactionHash?

    var imported: Boolean
        private set(value): Unit = set(16, value)
        get(): Boolean = get(16) as Boolean

    var deleted: Boolean
        private set(value): Unit = set(17, value)
        get(): Boolean = get(17) as Boolean

    var proxy: Boolean
        private set(value): Unit = set(18, value)
        get(): Boolean = get(18) as Boolean

    var implementationContractAddress: ContractAddress?
        private set(value): Unit = set(19, value)
        get(): ContractAddress? = get(19) as ContractAddress?
    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    override fun key(): Record1<UUID?> = super.key() as Record1<UUID?>

    constructor(id: UUID, alias: String, contractMetadataId: UUID, contractData: ContractBinaryData, constructorParams: JsonNode, initialEthAmount: Balance, chainId: ChainId, redirectUrl: String, projectId: UUID, createdAt: UtcDateTime, arbitraryData: JsonNode?, screenBeforeActionMessage: String?, screenAfterActionMessage: String?, contractAddress: ContractAddress?, deployerAddress: WalletAddress?, txHash: TransactionHash?, imported: Boolean, deleted: Boolean, proxy: Boolean, implementationContractAddress: ContractAddress?): this() {
        this.id = id
        this.alias = alias
        this.contractMetadataId = contractMetadataId
        this.contractData = contractData
        this.constructorParams = constructorParams
        this.initialEthAmount = initialEthAmount
        this.chainId = chainId
        this.redirectUrl = redirectUrl
        this.projectId = projectId
        this.createdAt = createdAt
        this.arbitraryData = arbitraryData
        this.screenBeforeActionMessage = screenBeforeActionMessage
        this.screenAfterActionMessage = screenAfterActionMessage
        this.contractAddress = contractAddress
        this.deployerAddress = deployerAddress
        this.txHash = txHash
        this.imported = imported
        this.deleted = deleted
        this.proxy = proxy
        this.implementationContractAddress = implementationContractAddress
    }
}
